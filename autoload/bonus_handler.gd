...
signal bonus_applied(entity: Node, bonus_type: BonusType)
...
var _bonus_map: Dictionary[Node, Array] = {}
var _bonus_calculation_type: Dictionary[BonusType, BonusCalculationType] = {
...
func apply_bonus(entity: Node, bonus_type: BonusType, is_permanent: bool = false) -> void:
	var bonus = Bonus.new()
	...
	bonus_applied.emit(entity, bonus_type)

func get_bonus(entity: Node, bonus_type: BonusType) -> Variant:
	if not _bonus_map.has(entity):
		return 0
